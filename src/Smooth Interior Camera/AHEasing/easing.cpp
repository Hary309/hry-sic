...
{
	return static_cast<AHFloat>(static_cast<AHFloat>(p));
}

// Modeled after the parabola y = x^2
AHFloat QuadraticEaseIn(AHFloat p)
{
	return static_cast<AHFloat>(p * p);
}

// Modeled after the parabola y = -x^2 + 2x
AHFloat QuadraticEaseOut(AHFloat p)
{
	return static_cast<AHFloat>(-(p * (p - 2)));
}
...
AHFloat QuadraticEaseInOut(AHFloat p)
{
	if(p < 0.5)
	{
		return static_cast<AHFloat>(2 * p * p);
	}
	else
	{
		return static_cast<AHFloat>((-2 * p * p) + (4 * p) - 1);
	}
}
...
{
	return static_cast<AHFloat>(p * p * p);
}
...
{
	AHFloat f = (p - 1);
	return static_cast<AHFloat>(f * f * f + 1);
}
...
AHFloat CubicEaseInOut(AHFloat p)
{
	if(p < 0.5)
	{
		return static_cast<AHFloat>(4 * p * p * p);
	}
	else
	{
		AHFloat f = ((2 * p) - 2);
...
{
	return static_cast<AHFloat>(p * p * p * p);
}
...
{
	AHFloat f = (p - 1);
	return static_cast<AHFloat>(f * f * f * (1 - p) + 1);
}
...
AHFloat QuarticEaseInOut(AHFloat p) 
{
	if(p < 0.5)
	{
		return static_cast<AHFloat>(8 * p * p * p * p);
	}
	else
	{
		AHFloat f = (p - 1);
		return static_cast<AHFloat>(-8 * f * f * f * f + 1);
	}
}
...
{
	return static_cast<AHFloat>(p * p * p * p * p);
}
...
{
	AHFloat f = (p - 1);
	return static_cast<AHFloat>(f * f * f * f * f + 1);
}
...
AHFloat QuinticEaseInOut(AHFloat p) 
{
	if(p < 0.5)
	{
		return static_cast<AHFloat>(16 * p * p * p * p * p);
	}
	else
	{
		AHFloat f = ((2 * p) - 2);
...
{
	if(p == 0.0 || p == 1.0) return static_cast<AHFloat>(p);
